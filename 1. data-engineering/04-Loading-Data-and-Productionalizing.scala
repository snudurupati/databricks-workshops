...
// MAGIC
// MAGIC <img alt="Side Note" title="Side Note" style="vertical-align: text-bottom; position: relative; height:1.75em; top:0.05em; transform:rotate(15deg)" src="https://files.training.databricks.com/static/images/icon-note.webp"/> When writing data to DBFS, the best practice is to use Delta.
...
// MAGIC %md-sandbox
// MAGIC Write to Delta by calling the following method on a DataFrame: `.write.format("delta").save("mnt/<destination>.parquet")`.
// MAGIC
...
display(dbutils.fs.ls("/tmp/" + username + "/delta/crime"))
...
crimeRenamedColsDF
          ...
          .format("delta")
          .save("/tmp/" + username + "/delta/crimeRepartitioned")
...
display(dbutils.fs.ls("/tmp/" + username + "/delta/crimeRepartitioned"))
...
// MAGIC ## Next Steps
// MAGIC
// MAGIC Full course available @ Databricks Academy, [Databricks Training](https://databricks.com/training).  
